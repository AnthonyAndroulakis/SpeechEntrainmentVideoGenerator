...
if __name__ == '__main__':
    ...
    net = TOFlow(intPreprocessedHeight, intPreprocessedWidth, task, cuda_flag=CUDA)
    if CUDA:
        net.load_state_dict(torch.load(os.path.join(workplace, 'toflow_models', model_name + '.pkl')))
        net.eval().cuda()
    else:
        net.load_state_dict(torch.load(os.path.join(workplace, 'toflow_models', model_name + '.pkl'), map_location=torch.device('cpu')))
        net.eval()
    ...
    try:
        ...
        predictshape = predict.shape
        processpredict = predict.flatten().tolist()
        outofplaceind = [i for i in range(len(processpredict)) if not 0<=processpredict[i]<=1]
        for j in outofplaceind:
            processpredict[j] = float(min([0,1], key=lambda x:abs(x-processpredict[j])))
        fixedpredict = np.array(processpredict).reshape(predictshape)
        plt.imsave(frameOutName, fixedpredict)
